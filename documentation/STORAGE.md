...
### Пример: Загрузка документа при регистрации Member
...
// После создания Member
const documentFile = await this.storageService.uploadFile({
    file: dto.documentFirstFile, // Express.Multer.File
    userId: user.id,
    category: FileCategory.MEMBER_DOCUMENT,
});

// Сохраняем путь в IdentityDocument
await this.identityDocumentRepository.create({
    memberId: member.id,
    type: dto.documentType,
    side: 'first',
    storagePath: documentFile.path, // или documentFile.url
});
```
